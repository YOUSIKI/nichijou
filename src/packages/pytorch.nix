{
  ...
  recipe = {
    ...
    inherit (darwinPackages) Accelerate CoreServices;
    inherit
      (pythonPackages)
      buildPythonPackage
      cffi
    ...
    # https://github.com/pytorch/pytorch/blob/v2.1.0/torch/utils/cpp_extension.py#L1951
  ...
  in
    buildPythonPackage rec {
      inherit (sources.pytorch) pname version src;
      format = "setuptools";
      ...
      buildInputs =
        [blas blas.provider pybind11]
        ++ lib.optionals stdenv.isLinux [linuxHeaders] # TMP: avoid "flexible array member" errors for now
        ++ lib.optionals cudaSupport (with cudaPackages;
          [
  ...
  package = pkgs.callPackage recipe {
    inherit sources;
    python = pkgs.python311;
    pythonPackages = pkgs.python311Packages;
    darwinPackages = pkgs.darwin;
    cudaSupport = true;
  };
