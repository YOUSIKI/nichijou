...
# to /etc/nixos/configuration.nix instead.
{
  ...
  systemd.services.mount-data-volume = let
    ...
    device = "/dev/nvme0n1p3:/dev/sda1:/dev/sdb1";
    target = "/mnt/data";
    options = "noatime";
  in {
    description = "Mount bcachefs volume ${target}";
    bindsTo = devices;
    after = devices ++ ["local-fs-pre.target"];
    ...
    serviceConfig = {
      ...
      ExecStart = "${pkgs.util-linux}/bin/mount -t bcachefs -o ${options} ${device} ${target}";
